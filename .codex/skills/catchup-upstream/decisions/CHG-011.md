...
# Upstream変更: remote compaction の見積もり/ペイロード整合

## 1. 観察事実（upstreamで何が変わったか）

- 変更内容:
- remote compaction の token 見積もりと payload 生成で base instructions を統一。
- upstreamの根拠:
  - 該当PR/コミット/リリースノート:
    - dc7007bea
  - 仕様/挙動の要点（箇条書き）:
...
- 方針: 採用
- 理由（制約・コスト・安全性・整合性）: コンパクションの一貫性向上。
- 移行方針（必要なら）: なし

### 3.2 VSCode拡張

- 方針: 未対応
- 理由（upstream整合・UX・実装制約）: 拡張 UI への直接影響なし。
...
- CLI/TUI:
- remote compaction の base instructions が統一される。
- VSCode:
- なし
