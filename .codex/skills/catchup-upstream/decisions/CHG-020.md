...
# Upstream変更: .agents を read-only 扱いに変更

## 1. 観察事実（upstreamで何が変わったか）

- 変更内容:
- .agents を .codex 同様に read-only 扱いに変更。
- upstreamの根拠:
  - 該当PR/コミット/リリースノート:
    - 9a487f9c1
  - 仕様/挙動の要点（箇条書き）:
...
- ドキュメント更新:
  - `README_codez.md`:
- README_codez.md に扱いの差分を追記（必要なら）。
